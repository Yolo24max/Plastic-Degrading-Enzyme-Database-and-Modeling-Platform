...
class PlaszymePredictor:
    ...
    def _init_protein_builder(self):
        ...
        if backbone_type == "GNN":
            builder_cfg = GNNBuilderConfig(
                ...
                radius=10.0,
                embedder=[{"name": "esm", "model_path": r"C:\Users\24683\Downloads\esm1b_t33_650M_UR50S.pt"}]
            )
    ...
    def _get_plastic_features(self, plastic_items: List[str]) -> Tuple[torch.Tensor, List[str]]:
        ...
        # 维度对齐 - 确保与训练时的维度一致（404维）
        EXPECTED_DIM = 404  # 训练时使用的特征维度
        max_dim = max(f.shape[0] for f in features)
        target_dim = max(max_dim, EXPECTED_DIM)  # 使用训练时的维度或更大的维度
        ...
        for feat in features:
            if feat.shape[0] < target_dim:
                ...
                padded_features.append(feat[:target_dim])  # 如果特征维度超过预期，截断

        print(f"[PLASTIC] Features extracted successfully | count={len(valid_names)} | dim={target_dim}")
