#!/bin/bash
# 更新 API 文件 v3 - 使数据库配置可选
echo "============================================"
echo "更新 API 文件 (v3 - 数据库可选)"
echo "============================================"

# 备份现有文件
if [ -f /var/www/html/api_protein_structure.php ]; then
    sudo cp /var/www/html/api_protein_structure.php /var/www/html/api_protein_structure.php.backup_v3_$(date +%Y%m%d_%H%M%S)
    echo "✓ 已备份现有文件"
fi
        ...
        case 'info':
            ...
            $info = [
                ...
                'files' => [
                    ...
                    'pdb_url' => 'api_protein_structure.php?plz_id=' . $plz_id . '&type=' . $data_type . '&action=pdb',
                    'json_url' => 'api_protein_structure.php?plz_id=' . $plz_id . '&type=' . $data_type . '&action=json'
                ]
            ];
            ...
            try {
                ...
                // 数据库信息可选，出错不影响主要功能
                $info['db_note'] = 'Database information not available';
            }
        ...
        case 'list_available':
            ...
            if (is_dir($pdb_dir)) {
                ...
                foreach ($pdb_files as $file) {
                    $xid_name = basename($file, '.pdb');
                    $available[] = [
                        'xid' => $xid_name,
                        'has_pdb' => true,
                        'has_json' => file_exists($json_dir . $xid_name . '.json')
                    ];
                }
            }
...
echo "测试2: 获取PDB文件 (前10行)"
curl -s "http://localhost/api_protein_structure.php?plz_id=866554aa77&type=predicted&action=pdb" | head -10
echo ""
echo ""

echo "测试3: 测试之前报错的 PLZ_ID"
